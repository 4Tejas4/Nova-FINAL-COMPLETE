...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState:Bundle?){super.onCreate(savedInstanceState);AppContext.init(this);setContentView(R.layout.activity_main)
        ...
        addBubble("Nova","I’m ready. Load a GGUF model in Settings, then say “Hey Nova” or type a command. Everything in this build is designed to reason locally on the phone.",false)
        updateState()
    }
    private fun sendText(){val text=input.text.toString().trim();if(text.isBlank())return;input.setText("");addBubble(text,"You"=="You".let{true});runCommand(text)}
