...
class NovaSecurityAudit(private val context: Context) {
    ...
    fun audit(): List<Finding> {
        ...
        out += Finding("accessibility", if (NovaAccessibilityService.get() == null) Severity.WARNING else Severity.INFO,
            "Accessibility automation", if (NovaAccessibilityService.get() == null) "Not connected. UI automation is limited." else "Connected.")
